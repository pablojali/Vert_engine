...
----------------------------------------------------
data (from data_mapper.build_report_data) -> interpretation model ->
visual components -> PDF bytes.

Adapted from Report/render_pdf.py (the mockup): identical page layouts,
but everything runs in memory - fonts load from this package's own
fonts/ folder, charts come from make_charts.build_charts() as BytesIO
(never touch disk), and build_pdf() returns bytes instead of writing a
file path. Never writes to output/ or anywhere the Builder publishes.
"""
...
def _img(charts, name):
    return ImageReader(charts[name])


# ================================================================= PAGE 1
def page1(c, data, model, charts):
    ...
    items = [
        ...
        ("OVERALL", f"#{res['overall_rank']}" if res.get("overall_rank") else "—"),
        ("CATEGORY", data["athlete"]["category"] or "—"),
    ]
...
def page4(c, data, model, charts):
    ...
    chart_h = 118
    y -= 6
    c.drawImage(_img(charts, "position_progression"), x, y - chart_h, width=w, height=chart_h,
                preserveAspectRatio=False, mask="auto")
    y -= chart_h + 16
...
def page5(c, data, model, charts):
    ...
    y -= 18
    # model["summary_paragraph"] is editable by hand before generating the
    # PDF (Report tab's text area) - build_pdf()'s summary_override
    # replaces it here; nothing else on this page changes as a result.
